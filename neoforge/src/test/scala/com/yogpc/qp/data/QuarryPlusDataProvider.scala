...
object QuarryPlusDataProvider {
  ...
  def gatherData(event: GatherDataEvent): Unit = {
    ...
    event.getGenerator.addProvider(event.includeServer, new Recipe(event.getGenerator, event.getLookupProvider))
    event.getGenerator.addProvider(event.includeServer, new RecipeAdvancement(event.getGenerator, event.getLookupProvider))
    event.getGenerator.addProvider(event.includeServer, new DefaultMachineConfig(event.getGenerator, event.getLookupProvider))
