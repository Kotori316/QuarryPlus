...
class MoverRecipeWrapper(recipe: MoverRecipe) extends IRecipeWrapper {
  ...
  override def getIngredients(ingredients: IIngredients): Unit = {
    ...
    ingredients.setInputLists(classOf[ItemStack], Collections.singletonList(input.asJava))
    ingredients.setOutputLists(classOf[ItemStack], Collections.singletonList(output.asJava))
  }
