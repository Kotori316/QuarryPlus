...
class GuiAdvQuarryFluid(tile: TileAdvQuarry, player: EntityPlayer, val facing: EnumFacing) extends GuiScreenA(null) {
    ...
    override def drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float): Unit = {
        ...
        drawCenteredString(this.fontRendererObj, I18n.format("FD." + facing), this.width / 2, 8, 0xFFFFFF)
    }
    ...
    class FluidSlot extends GuiSlot(this.mc, this.width * 3 / 5, this.height, 30, this.height - 30, 18) {
        ...
        override def drawSlot(slotIndex: Int, xPos: Int, yPos: Int, heightIn: Int, mouseXIn: Int, mouseYIn: Int): Unit = {
            filter.map(_.getLocalizedName).foreach(name =>
                mc.fontRendererObj.drawStringWithShadow(name, (GuiAdvQuarryFluid.this.width * 3 / 5 - mc.fontRendererObj.getStringWidth(name)) / 2, yPos + 2, 0xFFFFFF)
            )
        }
...
private class SelectFluid(parentAdv: GuiAdvQuarryFluid, tile: TileAdvQuarry) extends GuiScreenA(parentAdv) {
    ...
    override def drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float): Unit = {
        ...
        fluidList.drawScreen(mouseX, mouseY, partialTicks)
        super.drawScreen(mouseX, mouseY, partialTicks)
        drawCenteredString(this.fontRendererObj, I18n.format("FD." + parentAdv.facing), this.width / 2, 8, 0xFFFFFF)
    }
    ...
    class FluidSlot extends GuiSlot(this.mc, this.width, this.height, 24, this.height - 32, 18) {
        ...
        override def drawSlot(slotIndex: Int, xPos: Int, yPos: Int, heightIn: Int, mouseXIn: Int, mouseYIn: Int): Unit = {
            ...
            mc.fontRendererObj.drawStringWithShadow(name, (SelectFluid.this.width - mc.fontRendererObj.getStringWidth(name)) / 2, yPos + 2, 0xFFFFFF)
        }
