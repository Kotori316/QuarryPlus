...
class ListLikeMap[A, B] private(private val list: util.ArrayList[(A, B)]) {
  ...
  def foldRight[C](z: C)(op: ((A, B), C) => C): C = {
    var i = list.size() - 1
    var obj = z
    while (i >= 0) {
      obj = op(list.get(i), obj)
      i -= 1
    }
    obj
  }

  def map[C](f: (A, B) => C): List[C] = foldRight(List.empty[C]) { case (t, l) => f(t._1, t._2) :: l }

  def toList: List[(A, B)] = foldRight(List.empty[(A, B)]) { case (t, l) => t :: l }
