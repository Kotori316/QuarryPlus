...
public class RenderMarker implements BlockEntityRenderer<TileMarker> {
    ...
    public static Box[] getRenderBox(Area area) {
        ...
        if (zMin != zMax)
            flag |= 4;
        net.minecraft.world.phys.AABB[] lineBoxes = new net.minecraft.world.phys.AABB[12];
        if ((flag & 1) == 1) {//x
            lineBoxes[0] = new net.minecraft.world.phys.AABB(xMin + b, yMin + a, zMin + a, xMax + c, yMin + a, zMin + a);
        }
        if ((flag & 2) == 2) {//y
            lineBoxes[4] = new net.minecraft.world.phys.AABB(xMin + a, yMin + b, zMin + a, xMin + a, yMax + c, zMin + a);
        }
        if ((flag & 4) == 4) {//z
            lineBoxes[8] = new net.minecraft.world.phys.AABB(xMin + a, yMin + a, zMin + b, xMin + a, yMin + a, zMax + c);
        }
        if ((flag & 3) == 3) {//xy
            lineBoxes[2] = new net.minecraft.world.phys.AABB(xMin + b, yMax + a, zMin + a, xMax + c, yMax + a, zMin + a);
            lineBoxes[6] = new net.minecraft.world.phys.AABB(xMax + a, yMin + b, zMin + a, xMax + a, yMax + c, zMin + a);
        }
        if ((flag & 5) == 5) {//xz
            lineBoxes[1] = new net.minecraft.world.phys.AABB(xMin + b, yMin + a, zMax + a, xMax + c, yMin + a, zMax + a);
            lineBoxes[9] = new net.minecraft.world.phys.AABB(xMax + a, yMin + a, zMin + b, xMax + a, yMin + a, zMax + c);
        }
        if ((flag & 6) == 6) {//yz
            lineBoxes[5] = new net.minecraft.world.phys.AABB(xMin + a, yMin + b, zMax + a, xMin + a, yMax + c, zMax + a);
            lineBoxes[10] = new net.minecraft.world.phys.AABB(xMin + a, yMax + a, zMin + b, xMin + a, yMax + a, zMax + c);
        }
        if ((flag & 7) == 7) {//xyz
            lineBoxes[3] = new net.minecraft.world.phys.AABB(xMin + b, yMax + a, zMax + a, xMax + c, yMax + a, zMax + a);
            lineBoxes[7] = new net.minecraft.world.phys.AABB(xMax + a, yMin + b, zMax + a, xMax + a, yMax + c, zMax + a);
            lineBoxes[11] = new net.minecraft.world.phys.AABB(xMax + a, yMax + a, zMin + b, xMax + a, yMax + a, zMax + c);
        }
