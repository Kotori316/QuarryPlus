...
sealed class Box(val startX: Double, val startY: Double, val startZ: Double,
  ...
  def render(buffer: BufferBuilder, sprite: TextureAtlasSprite)(implicit lightValue: Box.LightValue): Unit = {
    val n1X = dx
...
private class BoxX(startX: Double,
  ...
  override def render(b: BufferBuilder, sprite: TextureAtlasSprite)(implicit lv: Box.LightValue): Unit = {
