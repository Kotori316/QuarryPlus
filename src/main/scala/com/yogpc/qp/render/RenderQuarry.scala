...
object RenderQuarry extends FastTESR[TileQuarry] {
    ...
    override def renderTileEntityFast(quarry: TileQuarry, distanceX: Double, distanceY: Double, distanceZ: Double,
                                      partialTicks: Float, destroyStage: Int, bufferBuilder: VertexBuffer) = {
        ...
        if ((quarry.G_getNow == TileQuarry.Mode.NOTNEEDBREAK || quarry.G_getNow == TileQuarry.Mode.MAKEFRAME) && quarry.yMax != Integer.MIN_VALUE) {
            ...
            for (i <- 0 until subtract.getX) {
                ...
                def tempY(y: Double): Unit = {
                    buffer.pos(minX + di, y - d1, minZ - d1).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, y - d1, minZ - d1).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, y - d1, minZ + d1).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(minX + di, y - d1, minZ + d1).colored().tex(V_minU, V_maxV).lightedAndEnd()

                    buffer.pos(minX + di, y - d1, maxZ - d1).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, y - d1, maxZ - d1).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, y - d1, maxZ + d1).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(minX + di, y - d1, maxZ + d1).colored().tex(V_minU, V_maxV).lightedAndEnd()
                ...
                def tempZ(z: Double): Unit = {
                    buffer.pos(minX + di, minY + d1, z - d1).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, minY + d1, z - d1).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, minY - d1, z - d1).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(minX + di, minY - d1, z - d1).colored().tex(V_minU, V_maxV).lightedAndEnd()

                    buffer.pos(minX + di, maxY + d1, z - d1).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, maxY + d1, z - d1).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(minX + dn, maxY - d1, z - d1).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(minX + di, maxY - d1, z - d1).colored().tex(V_minU, V_maxV).lightedAndEnd()
            ...
            for (i <- 0 until subtract.getY) {
                ...
                buffer.pos(minX - d1, minY + d1 + i + 0, minZ - d1).colored().tex(H_minU, H_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY + d1 + i + 0, minZ - d1).colored().tex(H_maxU, H_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY + d1 + i + n, minZ - d1).colored().tex(H_maxU, H_maxV).lightedAndEnd()
                buffer.pos(minX - d1, minY + d1 + i + n, minZ - d1).colored().tex(H_minU, H_maxV).lightedAndEnd()

                buffer.pos(maxX - d1, minY + d1 + i + 0, minZ - d1).colored().tex(H_minU, H_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY + d1 + i + 0, minZ - d1).colored().tex(H_maxU, H_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY + d1 + i + n, minZ - d1).colored().tex(H_maxU, H_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, minY + d1 + i + n, minZ - d1).colored().tex(H_minU, H_maxV).lightedAndEnd()

                buffer.pos(minX - d1, minY + d1 + i + 0, maxZ - d1).colored().tex(H_minU, H_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY + d1 + i + 0, maxZ - d1).colored().tex(H_maxU, H_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY + d1 + i + n, maxZ - d1).colored().tex(H_maxU, H_maxV).lightedAndEnd()
                buffer.pos(minX - d1, minY + d1 + i + n, maxZ - d1).colored().tex(H_minU, H_maxV).lightedAndEnd()

                buffer.pos(maxX - d1, minY + d1 + i + 0, maxZ - d1).colored().tex(H_minU, H_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY + d1 + i + 0, maxZ - d1).colored().tex(H_maxU, H_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY + d1 + i + n, maxZ - d1).colored().tex(H_maxU, H_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, minY + d1 + i + n, maxZ - d1).colored().tex(H_minU, H_maxV).lightedAndEnd()
            ...
            for (i <- 0 until subtract.getZ) {
                ...
                def tempY(y: Double): Unit = {
                    buffer.pos(minX - d1, y - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(minX - d1, y - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(minX + d1, y - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(minX + d1, y - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_maxV).lightedAndEnd()

                    buffer.pos(maxX - d1, y - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(maxX - d1, y - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(maxX + d1, y - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(maxX + d1, y - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_maxV).lightedAndEnd()
                ...
                def tempX(x: Double): Unit = {
                    buffer.pos(x - d1, minY + d1, minZ + d1 + i + 0).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(x - d1, minY + d1, minZ + d1 + i + n).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(x - d1, minY - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(x - d1, minY - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_maxV).lightedAndEnd()

                    buffer.pos(x - d1, maxY + d1, minZ + d1 + i + 0).colored().tex(V_minU, V_minV).lightedAndEnd()
                    buffer.pos(x - d1, maxY + d1, minZ + d1 + i + n).colored().tex(V_maxU, V_minV).lightedAndEnd()
                    buffer.pos(x - d1, maxY - d1, minZ + d1 + i + n).colored().tex(V_maxU, V_maxV).lightedAndEnd()
                    buffer.pos(x - d1, maxY - d1, minZ + d1 + i + 0).colored().tex(V_minU, V_maxV).lightedAndEnd()
            ...
            def renderBox(): Unit = {
                ...
                //z
                buffer.pos(minX + d1, minY + d1, minZ - d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY - d1, minZ - d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(minX - d1, minY - d1, minZ - d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(minX - d1, minY + d1, minZ - d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(maxX + d1, minY + d1, minZ - d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY - d1, minZ - d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(maxX - d1, minY - d1, minZ - d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, minY + d1, minZ - d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(minX + d1, minY + d1, maxZ + d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(minX + d1, minY - d1, maxZ + d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(minX - d1, minY - d1, maxZ + d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(minX - d1, minY + d1, maxZ + d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(maxX + d1, minY + d1, maxZ + d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(maxX + d1, minY - d1, maxZ + d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(maxX - d1, minY - d1, maxZ + d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, minY + d1, maxZ + d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(minX + d1, maxY + d1, minZ - d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(minX + d1, maxY - d1, minZ - d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(minX - d1, maxY - d1, minZ - d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(minX - d1, maxY + d1, minZ - d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(maxX + d1, maxY + d1, minZ - d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(maxX + d1, maxY - d1, minZ - d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(maxX - d1, maxY - d1, minZ - d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, maxY + d1, minZ - d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(minX + d1, maxY + d1, maxZ + d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(minX + d1, maxY - d1, maxZ + d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(minX - d1, maxY - d1, maxZ + d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(minX - d1, maxY + d1, maxZ + d1).colored().tex(B_minU, B_maxV).lightedAndEnd()

                buffer.pos(maxX + d1, maxY + d1, maxZ + d1).colored().tex(B_minU, B_minV).lightedAndEnd()
                buffer.pos(maxX + d1, maxY - d1, maxZ + d1).colored().tex(B_maxU, B_minV).lightedAndEnd()
                buffer.pos(maxX - d1, maxY - d1, maxZ + d1).colored().tex(B_maxU, B_maxV).lightedAndEnd()
                buffer.pos(maxX - d1, maxY + d1, maxZ + d1).colored().tex(B_minU, B_maxV).lightedAndEnd()
