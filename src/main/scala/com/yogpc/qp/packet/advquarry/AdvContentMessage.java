...
public class AdvContentMessage implements IMessage {
    ...
    public void fromBytes(PacketBuffer buffer) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            FluidStack stack = FluidStack.loadFluidStackFromNBT(buffer.readNBTTagCompoundFromBuffer());
            FluidTank tank = new FluidTank(null, 0);
            Optional.ofNullable(buffer.readNBTTagCompoundFromBuffer()).ifPresent(tank::readFromNBT);
    ...
    public void toBytes(PacketBuffer buffer) {
        ...
        map.forEach((fluidStack, fluidTank) -> {
            ...
            buffer.writeNBTTagCompoundToBuffer(fluidTank.writeToNBT(new NBTTagCompound()));
        });
    }
