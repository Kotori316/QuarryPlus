...
object ItemTemplate {
  ...
  val enchantmentName: Kleisli[List, ItemStack, TranslationTextComponent] = Kleisli((stack: ItemStack) => List(silktouchName, fortuneName).flatMap(_.apply(stack)))
  ...
  val blocksList: Kleisli[Option, (ItemStack, TileBasic), NoDuplicateList[BlockData]] = Kleisli((t: (ItemStack, TileBasic)) => {
    val (stack: ItemStack, basic: TileBasic) = t
    silkList(stack, basic) orElse fList(stack, basic)
  })
  val includeSetter: Kleisli[Option, (ItemStack, TileBasic), Boolean => Unit] = Kleisli((t: (ItemStack, TileBasic)) => {
    val (stack: ItemStack, basic: TileBasic) = t
