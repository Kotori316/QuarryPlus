...
class EnergyDebug(tile: APowerTile) {
  ...
  def use(amount: Long, simulate: Boolean, usage: EnergyUsage): Unit = {
    ...
    totalUsed += amount
    usageMap(usage) = usageMap.getOrElse(usage, 0L) + amount
  }
    ...
    if (allUsed != 0 || allGot != 0) {
      if (allUsed == 0 || usedCount == 0) {
        ...
        if (gotCount == 0)
          ...
          QuarryPlus.LOGGER.info(
            s"$tileName used $allUsed MJ in $usedCount ticks (${allUsed * 10 / usedCount} RF/t), got $allGot NJ in $gotCount ticks (${allGot * 10 / gotCount} RF/t)"
          )
      }
    }
    usedCount = 0
