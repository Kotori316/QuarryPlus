...
class TileExpPump extends APacketTile with IEnchantableTile with IDebugSender {
    ...
    private def refreshConnection() = {
        if (hasWorld && !worldObj.isRemote) {
            val (facing, _) = EnumFacing.VALUES
              .map(f => (f, getWorld.getTileEntity(getPos.offset(f))))
              .collectFirst { case (f: EnumFacing, t: TileBasic) if t.S_connectExppump(f.getOpposite) => (f, t) }
              .getOrElse((null, null))
            setConnectTo(facing)
            S_sendNowPacket()
        }
    ...
    def addXp(amount: Int): Unit = {
        xpAmount += amount
        if (getWorld.getBlockState(getPos).getBlock != QuarryPlusI.blockExpPump)
            return
