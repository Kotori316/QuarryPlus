...
class TileAdvQuarry extends APowerTile with IEnchantableTile with HasInv with ITickable with IDebugSender with IChunkLoadTile {
    ...
    private def chunkLoad() = {
        if (chunks.nonEmpty) {
            ...
            val bool = getWorld.getChunkProvider.getLoadedChunk(chunkPos.chunkXPos, chunkPos.chunkZPos) == null
            if (Config.content.debug) {
                QuarryPlus.LOGGER.debug("Chunk has already loaded : " + bool + chunkPos.chunkXPos + chunkPos.chunkZPos)
            }
            if (!bool)
                getWorld.getChunkFromChunkCoords(chunkPos.chunkXPos, chunkPos.chunkZPos)
            chunks = chunks.tail
        }
    }
...
object TileAdvQuarry {
    ...
    private[TileAdvQuarry] class NotNullList(val seq: mutable.Buffer[ItemStack] with Clearable) extends NonNullList[ItemStack](seq.asJava, null, false) {
        var fix = false
