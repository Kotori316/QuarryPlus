...
object Config {
  ...
  class Content(builder: ForgeConfigSpec.Builder) {
    ...
    val collectBedrock: ForgeConfigSpec.BooleanValue = addBoolOption(builder.comment("True to enable ChunkDestroyer to collect bedrock as item if the machine has Bedrock Module in its module slots.").define("CollectBedrock", false))
