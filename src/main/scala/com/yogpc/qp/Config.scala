...
object Config {
  ...
  class Content(builder: ForgeConfigSpec.Builder) {
    ...
    val removeBedrock: ForgeConfigSpec.BooleanValue = addBoolOption(builder.comment("True to allow machines to remove bedrock. (Just removing. Not collecting)").define("RemoveBedrock", inDev))
